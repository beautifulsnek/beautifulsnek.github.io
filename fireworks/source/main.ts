function noop() { }
function constant(x: any) { return function (...args: any[]) { return x; } }
...
type ParticleCallback = ((x: number, y: number, vx: number, vy: number) => void);
...
var OFFSET_X = 0,
 ...
 OFFSET_VY = 6,
 OFFSET_VS = 7,
 PARTICLE_DATA_LENGTH = 8;
class Fireworks {
 protected particles: number[] = [];
 protected particleCallbacks: ParticleCallback[] = [];

 protected nextPurgePos = 0;
 protected purgeCountdownTime = 2;
 protected nextPurgeCountdown = this.purgeCountdownTime;
 

 constructor(
  public bounds: number[],
  public gravity: number[] = [0, 10],
  public palette: { [key: number]: string } = {
   [COLOR_WHITE]: '#FFFFFF',
   [COLOR_RED]: '#FF2200',
   [COLOR_GREEN]: '#00FF33',
   [COLOR_BLUE]: '#2266FF',
 ...
 addParticle(x: number, y: number, color: number, s: number, life: number, vx: number, vy: number, vs: number, callback: ParticleCallback = noop) {
  this.particles.push(x, y, color, s, life, vx, vy, vs);
  this.particleCallbacks.push(callback);
 }

 purge(maxViewed: number = 1000, maxRemoved: number = 100) {
  var particles = this.particles,
   len = particles.length - this.nextPurgePos,
   start = this.nextPurgePos < len ? this.nextPurgePos : 0,
   boundLeft = this.bounds[3],
   boundTop = this.bounds[0],
   boundRight = this.bounds[1],
   boundBottom = this.bounds[2],
   viewedCount = 0,
   removedCount = 0,
   particleX, particleY, particleLife;
 ...
 render(ctx: CanvasRenderingContext2D) {
  ...
  for (var i = 0; i < len; i += PARTICLE_DATA_LENGTH) {
   if (particles[i + OFFSET_LIFE] <= 0) continue;
   var particleSize = particles[i + OFFSET_S];

   ctx.fillStyle = this.palette[particles[i + OFFSET_COLOR]];

   ctx.fillRect(
    particles[i + OFFSET_X],
    particles[i + OFFSET_Y],
    particleSize,
    particleSize
   );

   // TODO: Maybe not bad, not sure
   /*ctx.fillRect(
    particles[i + OFFSET_X] - particles[i + OFFSET_VX] (1 / 30) + (Math.random() 4 - 2),
    particles[i + OFFSET_Y] - particles[i + OFFSET_VY] (1 / 30) + (Math.random() 4 - 2),
    1, 1
   );
   ctx.fillRect(
    particles[i + OFFSET_X] - particles[i + OFFSET_VX] (1 / 15) + (Math.random() 4 - 2),
    particles[i + OFFSET_Y] - particles[i + OFFSET_VY] (1 / 15) + (Math.random() 4 - 2),
    1, 1
   );*/
   //if (i === 0) console.log(particles[i + OFFSET_X]);
  }
 ...
 update(dt: number) {
  ...
  for (var i = 0; i < len; i += PARTICLE_DATA_LENGTH) {
   ...
   if (particles[i + OFFSET_S] < 0) particles[i + OFFSET_S] = 0;

   particles[i + OFFSET_LIFE] -= dt;
   if (particles[i + OFFSET_LIFE] <= 0) {
    this.particleCallbacks[i / PARTICLE_DATA_LENGTH](
     particles[i + OFFSET_X],
     particles[i + OFFSET_Y],
     particles[i + OFFSET_VX],
     particles[i + OFFSET_VY]
    );
   }
   ...
   // Air drag
   // TODO: Either looks weird or does not affect looks much
   /*var vx = particles[i + OFFSET_VX];
   var vy = particles[i + OFFSET_VY];
   var velSqr = (vx vx + vy vy);

   particles[i + OFFSET_VX] += -Math.sign(vx) (0.001 velSqr) * dt;
   particles[i + OFFSET_VY] += -Math.sign(vy) (0.001 velSqr) * dt;
   */
  }
...
function randreal(low: number, high?: number) {
 if (high === void 0) {
  high = low;
  low = 0;
 }
...
function randomFloatRangeGetter(range: number[]) {
 return function () {
  return randreal(range[0], range[1]);
 }
}
function randomRangeGetter(range: number[]) { return function () { return randint(range[0], range[1]); } }
function randomSequenceGetter<T>(sequence: T[]): () => T {
 return function() { return sequence[Math.floor(Math.random() * sequence.length)]; };
}
function multiplyFn(k: number, f: () => number) {
 return function () {
  return f() * k;
 }
}

function spawnRingExplosion(fireworks: Fireworks, x: number, y: number, particleCount: number = 60, depth = 1, velocityRange: number[] = [40, 60], life?: number) {
...
interface ExplosionData {
 ...
 velocityAddition?: number[] | (() => number[]);

 particleCount?: number | number[] | (() => number);
 depth?: number;

 velocityCoeff?: number;
 lifeCoeff?: number;
 sizeCoeff?: number;
 particleCountCoeff?: number;
}

function spawnExplosion(fireworks: Fireworks, data: ExplosionData) {
 var getNewParticleX = {
  'number': constant(data.x),
  'object': randomRangeGetter(data.x as number[]),
  'function': data.x
 }[typeof data.x];
 var getNewParticleY = {
  'number': constant(data.y),
  'object': randomRangeGetter(data.y as number[]),
  'function': data.y
 }[typeof data.y];
 var getNewParticleV = {
  'number': constant(data.velocity),
  'object': randomFloatRangeGetter(data.velocity as number[]),
 ...
 var getParticleCount = {
  ...
  'object': randomRangeGetter(data.particleCount as number[]),
...
function spawnBall(fireworks: Fireworks, centerX: number, centerY: number, radius: number, particleCount: number = 30) {
 for (var i = 0; i < particleCount; ++i) {
...
class FireworkShell {
    constructor(
        ...
        public readonly subshells: [FireworkShell, Point2d][]
    ) {

 }
}

function fireFromTo(fireworks: Fireworks, startPos: Point2d, endPos: Point2d, arrivalTime: number, color: number, size: number, vs: number = size / arrivalTime, callback?: ParticleCallback) {
...
function fireFromToExplosion(fireworks: Fireworks, startPos: Point2d, endPos: Point2d, arrivalTime: number, endBurstSize: number) {
...
function getRandomHeartPoint(canvasSize: Point2d): Point2d {
...
function fireShell(fireworks: Fireworks, pos: Point2d, velocity: Point2d, shell: FireworkShell) {
...
function main() {
 ...
 var ctx = display.getContext('2d') as CanvasRenderingContext2D;

 var fireworks = new Fireworks([0, DISPLAY_WIDTH, DISPLAY_HEIGHT, 0], [0, 20]);

 function sceneA(fireworks: Fireworks) {
     var speed = -400;
 ...
 function sceneB(fireworks: Fireworks) {
     var count = 3;
     ...
     function setParticleTimeout(i: number) {
         var colors = {
  ...
  function sceneC(fireworks: Fireworks) {
     var count = 3;
     var xPadding = DISPLAY_WIDTH * 0.2;
     var xStart = xPadding;
     var xEnd = DISPLAY_WIDTH - xPadding;
 ...
 function sceneD(fireworks: Fireworks) {
 ...
 function sceneE(fireworks: Fireworks) {
     var cx = DISPLAY_WIDTH / 2;
     var speed = -400;
     var life = DISPLAY_HEIGHT * 0.7 / Math.abs(speed);
     playShortFlight();
 ...
 function sceneF(fireworks: Fireworks) {
     var cx = DISPLAY_WIDTH / 2;
 ...
 function sceneFinal(fireworks: Fireworks) {
 ...
 var scenes: ([number, (fireworks: Fireworks) => void])[] = [
     [5000, sceneA],
     [7000, sceneB],
     [5000, sceneC],
     [9000, sceneD],
 ...
 var calcSceneDelay = function (i: number) {
     return i < 1
         ? 200
         : scenes[i - 1][0] + calcSceneDelay(i - 1);
 };

 var setSceneTimeout = function(i: number) {
     setTimeout(function () { 
